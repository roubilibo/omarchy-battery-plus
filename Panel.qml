...
Panel {
  ...
  readonly property bool physicallyCharging: {
    var d = UPower.displayDevice
    return !!(d && d.isPresent && !UPower.onBattery && d.state === UPowerDeviceState.Charging)
  }
  ...
  readonly property color batteryTextColor: {
    var c = root.batteryStatusColor
    var luminance = 0.2126 * c.r + 0.7152 * c.g + 0.0722 * c.b
    // With a mostly empty battery the text sits on the dark background;
    // otherwise choose the opposite lightness of the colored fill.
    if (root.batteryFraction < 0.50) return "#ffffff"
  ...
  Component {
    ...
    Item {
      ...
      Rectangle {
        width: Style.space(3)
        height: Math.max(Style.space(6), batteryBody.height * 0.32)
  ...
  BarIconButton {
    ...
    bar: root.bar
    text: ""
    iconComponent: batteryIconComponent
